...
{
    /// <summary>
    /// Wraps the functions contained in the DECtalk TTS engine.
    /// </summary>
    public class DECtalkEngine : IDisposable
    {
        #region Events
        /// <summary>
        ...
        [DllImport("DECtalk.dll")]
        static extern MMRESULT TextToSpeechSpeak(IntPtr handle,
            [MarshalAs(UnmanagedType.LPStr)] 
            string msg,
            uint flags);
        ...
        */
        ...
        public DECtalkEngine(string language, uint rate, TtsVoice speaker)
        {
            Init(language);
            Voice = speaker;
            Rate = rate;
        ...
        /// <param name="rate">The speaking rate to set.</param>
        /// <param name="speaker">The speaker voice to set.</param>
        public DECtalkEngine(uint rate, TtsVoice speaker)
        {
        ...
        {
            Check(TextToSpeechSpeak(_handle, msg, (uint)SpeakFlags.Force));
        }
        ...
        private static void Check(MMRESULT code)
        {
            ...
            {
                throw new DECtalkException(code);
            }
        }
        ...
        {
            private readonly DECtalkEngine _engine;

            public InMemoryRaiiHelper(DECtalkEngine engine)
            {
                _engine = engine;
            }
        ...
        private struct BufferRaiiHelper : IDisposable
        {
            ...
            public BufferRaiiHelper(DECtalkEngine engine)
            {
                _engine = engine;
            }
